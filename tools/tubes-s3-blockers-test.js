...
test('valid NodeMessage telemetry is sampled only by the pre-routing observer', () => {
  ...
  assert.equal((observer.match(/peerTelemetry\.observe\(/g) ?? []).length, 1);
  const peerData = nodeSource.match(/void onPeerData\([\s\S]*?MeshRoutePlan route/);
  assert.ok(peerData, 'peer data handler is present');
  assert.equal((peerData[0].match(/peerTelemetry\.observe\(/g) ?? []).length, 0);
});
