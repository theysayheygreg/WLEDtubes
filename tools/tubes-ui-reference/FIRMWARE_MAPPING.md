...
| `drawConductorTelemetry()` `patternName`, `bpm`, `beat`, `deviceId`, `deviceNumber` | CURRENT DEVICE card | S3 FD2 / #200 is the realistic fixture.
...
| `nodeId`, `deviceNumber`, `uplinkId`, `latestRssi`, `lastSeenMs` | Tube ID, device #, Following, signal, age | Fixture peer: 847 / -- / 000 / −28 dBm / 4s.
| scanner `status.deviceId` / `deviceNumber` | Separate scanner identity card | Not a row in its own survey list.
