...
, _data(new (std::nothrow) uint32_t[bc.count]())
{
  _valid = _data != nullptr;
}

void BusTubesNull::setPixelColor(unsigned pix, uint32_t c) {
  if (!_valid || pix >= _len) return;
  if (_reversed) pix = _len - pix - 1;
  _data[pix] = c;
}

uint32_t BusTubesNull::getPixelColor(unsigned pix) const {
  if (!_valid || pix >= _len) return 0;
  if (_reversed) pix = _len - pix - 1;
  return _data[pix];
}
...
#ifdef TUBES_NULL_OUTPUT
  // Never construct a transport, even if persisted configuration names one.
  uint8_t noPins[OUTPUT_MAX_PINS] = {255, 255, 255, 255, 255};
  BusConfig nullConfig(TYPE_TUBES_NULL, noPins, 0, PIXEL_COUNTS, COL_ORDER_RGB);
  busses.clear();
  busses.push_back(make_unique<BusTubesNull>(nullConfig));
  return busses.size();
#endif
