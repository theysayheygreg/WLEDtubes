...
The `waveshare_s3_tubes_remote` build compiles both spatial and mobile-conductor capabilities without defining `MASTER`. A fresh S3 is therefore authority-neutral. Its Conductor screen deliberately promotes or releases persisted `MasterRole` authority through the existing controller path; there is still no compile-time default authority policy. On this S3 field-OS build, route-sidecar origination is a separate runtime Anchor control that defaults off after every boot and can be enabled only while the node holds master authority. Existing non-S3 mobile-conductor builds retain their role-derived authority behavior.
