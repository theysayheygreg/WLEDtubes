...
class PatternController : public MessageReceiver {
...
#if defined(TUBES_ENABLE_HELLO_VFX) || defined(TUBES_ENABLE_HTTP_OTA_VFX)
    enum class TransientKind : uint8_t { None, Hello };
    TransientKind transientKind = TransientKind::None;
    uint32_t transientStarted = 0;
    TubesExperiment::HelloGate helloGate;
#endif
#ifdef TUBES_ENABLE_HTTP_OTA_VFX
    TubesExperiment::PurpleOtaState purpleOta;
#endif
    // AI: end
#ifdef TUBES_ENABLE_SPATIAL_PATTERNS
    // Local-only experiment selectors. They are never serialized as pattern_id.
    static constexpr uint8_t SPATIAL_LATENCY_FLOOR_ID = 240;
    static constexpr uint8_t SPATIAL_BPM_DRIFT_ID = 241;
    uint8_t spatialExperiment = SPATIAL_LATENCY_FLOOR_ID;
#endif
  ...
  void update()
  {
    ...
    if (helloGate.update(millis(), node.header.uplinkId != 0)) {
      transientKind = TransientKind::Hello;
      transientStarted = millis();
    }
#endif
...
#ifdef TUBES_ENABLE_HTTP_OTA_VFX
    if (purpleOta.shouldReboot(millis())) doReboot = true;
#endif
  ...
  void handleOverlayDraw() {
    ...
    // Experiment layers are computed directly into the strip last. The base
    // scheduler above keeps advancing, so clip expiry reveals its current frame.
    drawExperimentOverlay();
  }

  // AI: below section was generated by an AI
  void drawExperimentOverlay() {
#if defined(TUBES_ENABLE_HELLO_VFX) || defined(TUBES_ENABLE_HTTP_OTA_VFX) || defined(TUBES_ENABLE_SPATIAL_PATTERNS)
    const uint16_t length = strip.getLengthTotal();
    const uint32_t now = millis();
#ifdef TUBES_ENABLE_SPATIAL_PATTERNS
    if (spatialExperiment == SPATIAL_LATENCY_FLOOR_ID) {
      const uint32_t floor = TubesExperiment::latencyFloorMs(250, false, 0);
      const CRGB color = TubesExperiment::latencyEventOn(now, floor) ? CRGB(64, 0, 96) : CRGB::Black;
      for (uint16_t i = 0; i < length; i++) strip.setPixelColor(i, color);
    } else if (spatialExperiment == SPATIAL_BPM_DRIFT_ID) {
      const uint8_t shell = TubesExperiment::localShell(node.isFollowing(), node.isLeading());
      const uint32_t localBpm = TubesExperiment::shellBpm256(beats.bpm, shell, 2);
      const uint32_t phase = static_cast<uint32_t>((static_cast<uint64_t>(beats.frac) * localBpm) / beats.bpm);
      const uint8_t level = sin8(static_cast<uint8_t>(phase));
      for (uint16_t i = 0; i < length; i++) strip.setPixelColor(i, CRGB(level, 0, level));
    }
#endif
#if defined(TUBES_ENABLE_HELLO_VFX) || defined(TUBES_ENABLE_HTTP_OTA_VFX)
    if (transientKind == TransientKind::Hello) {
      const uint32_t elapsed = now - transientStarted;
      const uint16_t lit = TubesExperiment::helloLitPixels(elapsed, length);
      if (!lit) transientKind = TransientKind::None;
        ...
        strip.setPixelColor(TubesExperiment::logicalPixel(logical, length, false), color);
      }
    }
#endif
#ifdef TUBES_ENABLE_HTTP_OTA_VFX
    const TubesExperiment::PurplePhase phase = purpleOta.phaseAt(now);
    if (phase == TubesExperiment::PurplePhase::PrePulseOn || phase == TubesExperiment::PurplePhase::PrePulseOff ||
        phase == TubesExperiment::PurplePhase::SuccessPulseOn || phase == TubesExperiment::PurplePhase::SuccessPulseOff) {
      const bool on = phase == TubesExperiment::PurplePhase::PrePulseOn || phase == TubesExperiment::PurplePhase::SuccessPulseOn;
      for (uint16_t i = 0; i < length; i++) strip.setPixelColor(i, on ? CRGB(128, 0, 180) : CRGB::Black);
    }
#endif
#endif
  }
  ...
  void failHttpOtaVfx() { purpleOta.failed(); }
#endif

#ifdef TUBES_ENABLE_SPATIAL_PATTERNS
  bool selectSpatialExperiment(uint8_t id) {
    if (id != SPATIAL_LATENCY_FLOOR_ID && id != SPATIAL_BPM_DRIFT_ID) return false;
    spatialExperiment = id;
    return true;
  }
#endif
  // AI: end
